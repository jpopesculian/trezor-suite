...
import type { ConnectSettings, Device as DeviceTyped, CommonParams } from '../types';
 ...
 * @memberof Core
 */
const createUiPromise = <T extends UiPromise['type']>(promiseEvent: T, device?: IDevice) => {
    const uiPromise = createDeferred<Extract<UiPromise, { type: T }>>(promiseEvent, device);
 ...
 * @memberof Core
 */
const findUiPromise = <T extends UiPromise['type']>(promiseEvent: T) =>
    _uiPromises.find(p => p.id === promiseEvent) as
        | Deferred<Extract<UiPromise, { type: T }>>
        | undefined;
 ...
 * @memberof Core
 */
// REF-TODO const handleDeviceSelectionChanges = (interruptDevice?: ?DeviceTyped = null) => {
const handleDeviceSelectionChanges = (interruptDevice: any = null) => {
        ...
        _uiPromises.forEach(p => {
            if (p.device && p.device.getDevicePath() === path) {
                if (p.id === DEVICE.DISCONNECT) {
                    // @ts-ignore REF-TODO: refactor deferred "id" type to generic
                    p.resolve({ type: DEVICE.DISCONNECT, payload: undefined });
                }
...
const initDeviceList = async (settings: ConnectSettings) => {
    try {
        ...
        _deviceList.on(TRANSPORT.ERROR, async (error: any) => {
        ...
        _deviceList.on(TRANSPORT.START, (transportType: any) =>
